...
const Benchmark = ({ audits, onUpdateAudit, onRefresh }) => {
  ...
  // Get all notes and action items from an audit
  ...
  const handleSaveCompletion = async () => {
    ...
    const updatedAudit = {
      ...audit,
      sections: audit.sections.map((sec, si) => si !== completingItem.secIdx ? sec : {
        ...sec,
        items: sec.items.map((item, ii) => ii !== completingItem.itemIdx ? item : {
          ...item, actionCompleted: true, actionCompletedDate: completionDate
        })
      })
    };
                ...
                {!hasData ? (
                  ...
                  <>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      {oaScore && <div style={{ display: "flex", alignItems: "center", gap: 4 }}><TypeBadge type="OA" /><span style={{ fontWeight: 700, fontSize: 14, color: gradeColor(oaScore.pct) }}>{oaScore.pct}%</span>{oaDelta !== null && <Delta d={oaDelta} />}</div>}
                      {saScore && <div style={{ display: "flex", alignItems: "center", gap: 4 }}><TypeBadge type="SA" /><span style={{ fontWeight: 700, fontSize: 14, color: gradeColor(saScore.pct) }}>{saScore.pct}%</span></div>}
                      {openActions > 0 && <span style={{ fontSize: 11, color: "#DC2626", fontWeight: 700 }}>⚠ {openActions} open</span>}
                      {closedActions > 0 && <span style={{ fontSize: 11, color: "#16A34A", fontWeight: 700 }}>✓ {closedActions} done</span>}
                    </div>
                    <div style={{ fontSize: 11, color: "#94A3B8" }}>{siteAudits.length} audit{siteAudits.length !== 1 ? "s" : ""} · Latest: {fmt(siteAudits.sort((a,b) => b.submittedAt - a.submittedAt)[0]?.date)}</div>
                  </>
                )}
              </div>
              ...
              <div style={{ background: "#FAFBFC", border: "0.5px solid #E2E8F0", borderTop: "none", borderRadius: "0 0 12px 12px", padding: 14 }}>
                ...
                {latestOA && oaSections.length > 0 && (
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#1D4ED8", marginBottom: 8 }}>OA — {fmt(latestOA.date)} · Weighted: {calcScore(latestOA).total}/4.00</div>
